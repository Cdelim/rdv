...
#   1. the class name (GS3D_Ratio instead of GS3D), and
#   2. the shader `path=`, which points at decomposition_tracking_GS_ratio.h
...
# identical between the delta- and ratio-tracking estimators, only the
# per-ray blending shader differs.
#
...
# __init__.py:
#     from ._gaussian_splats_ratio import GS3D_Ratio
# so that `rdv.GS3D_Ratio` becomes available exactly like `rdv.GS3D`.
...
class GS3D_Ratio(_core.Map):
    __extension_info__ = dict(
        path=_core.__INCLUDE_PATH__ + '/volume_rendering/decomposition_tracking_GS_ratio.h',
        parameters=dict(
            ads=_torch.int64,
    ...
    def __init__(self, positions: _core.TensorLike | _core.deferred,
        ...
        assert len(positions.shape) == 2 and positions.shape[-1] == 3, "GS3D_Ratio requires positions to be of shape (N, 3)"
        assert len(colors.shape) == 2, "GS3D_Ratio requires colors to be of shape (N, C)"
        assert f_rest.shape[-1] == 45, f"GS3D_Ratio requires f_rest to have 45 columns, got {f_rest.shape[-1]}"

        if output_dim is None:
            output_dim = colors.shape[-1]
        assert output_dim == colors.shape[-1], f"GS3D_Ratio output_dim must match, got {output_dim} but colors has {colors.shape}"
        ...
        if input_dim is None:
            input_dim = 6
        assert input_dim == 6, f"GS3D_Ratio only supports input_dim=6 (x, w), got input_dim={input_dim}"
