...
class GS3D_Sampled(_core.Map):
    ...
    def __init__(self, positions: _core.TensorLike | _core.deferred,
        ...
        assert len(positions.shape) == 2 and positions.shape[-1] == 3, "GS3D_Sampled requires positions to be of shape (N, 3)"
        assert len(colors.shape) == 2, "GS3D_Sampled requires colors to be of shape (N, C)"
        assert f_rest.shape[-1] == 45, f"GS3D_Sampled requires f_rest to have 45 columns, got {f_rest.shape[-1]}"

        if output_dim is None:
            output_dim = colors.shape[-1]
        assert output_dim == colors.shape[-1], f"GS3D_Sampled output_dim must match, got {output_dim} but colors has {colors.shape}"
        ...
        if input_dim is None:
            input_dim = 6
        assert input_dim == 6, f"GS3D_Sampled only supports input_dim=6 (x, w), got input_dim={input_dim}"
