# _gaussian_splats_sun2025.py
#
# Python wrapper for the Sun et al. (EGSR 2025, arXiv:2504.06598) stochastic
# ray-tracing baseline -- see stochastic_raytracing_sun2025.h for the full
# comparison against this thesis's own estimators.
#
# Structurally a near-verbatim copy of _gaussian_splats.py; the ONLY
# functional differences from GS3D are:
#   1. the class name (DSYG_SunStochastic instead of GS3D), and
#   2. the shader `path=`, which points at stochastic_raytracing_sun2025.h
#      instead of decomposition_tracking_GS.h.
# The acceleration structure (BLAS/TLAS, 3-sigma AABBs) is deliberately
# UNCHANGED so that this baseline differs from the thesis's estimators only
# in the per-ray shading logic, not in the geometry it traverses.
#
# Register in rdv/__init__.py:
#     from ._gaussian_splats_sun2025 import DSYG_SunStochastic
...
# identical across all of GS3D, GS3D_Ratio, and DSYG_SunStochastic -- only the
...
# __init__.py:
#     from ._gaussian_splats_sampled import DSYG_SunStochastic
# so that `rdv.DSYG_SunStochastic` becomes available exactly like `rdv.GS3D`.
...
class DSYG_SunStochastic(_core.Map):
    __extension_info__ = dict(
        path=_core.__INCLUDE_PATH__ + '/volume_rendering/stochastic_raytracing_sun2025.h',
        parameters=dict(
            ads=_torch.int64,
        ...
        stochastic=True   # properly-sampled hit location -- genuinely stochastic per trial;
                          ...
                          # GS3D, not just one -- see stochastic_raytracing_sun2025.h.
    )
    ...
    def __init__(self, positions: _core.TensorLike | _core.deferred,
        ...
        assert len(positions.shape) == 2 and positions.shape[-1] == 3, "DSYG_SunStochastic requires positions to be of shape (N, 3)"
        assert len(colors.shape) == 2, "DSYG_SunStochastic requires colors to be of shape (N, C)"
        assert f_rest.shape[-1] == 45, f"DSYG_SunStochastic requires f_rest to have 45 columns, got {f_rest.shape[-1]}"

        if output_dim is None:
            output_dim = colors.shape[-1]
        assert output_dim == colors.shape[-1], f"DSYG_SunStochastic output_dim must match, got {output_dim} but colors has {colors.shape}"
        ...
        if input_dim is None:
            input_dim = 6
        assert input_dim == 6, f"DSYG_SunStochastic only supports input_dim=6 (x, w), got input_dim={input_dim}"
