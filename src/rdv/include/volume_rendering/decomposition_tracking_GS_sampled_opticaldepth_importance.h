...
THIS FILE: ALPHA_MODE=optical depth, SAMPLE_MODE=importance. One of six
...
a time. Each is wired to its own rdv.Map class in
_gaussian_splats_sampled.py:

  class                                 | shader file
  --------------------------------------|------------------------------------------------------------
  GS3D_Sampled                          | decomposition_tracking_GS_sampled.h                        (opticaldepth, volumetric -- default)
  GS3D_Sampled_OpticalDepth_Importance  | decomposition_tracking_GS_sampled_opticaldepth_importance.h
  GS3D_Sampled_OpticalDepth_Center      | decomposition_tracking_GS_sampled_opticaldepth_center.h
  GS3D_Sampled_Sun_Volumetric           | decomposition_tracking_GS_sampled_sun_volumetric.h
  GS3D_Sampled_Sun_Importance           | decomposition_tracking_GS_sampled_sun_importance.h
  GS3D_Sampled_Sun_Center                | decomposition_tracking_GS_sampled_sun_center.h
*/
            ...
            if (A > 1e-6) {
               ...
               // float disc = B*B - A*(C - K_SIGMA*K_SIGMA);
                ...
                if (power > POWER_CUTOFF) {
                    ...
                    // SAMPLE_MODE: importance sampling. Whether this
                    // primitive interacts is a Bernoulli test on `alpha`
                    // (computed above); given a hit, the position is one
                    // genuine draw from the Gaussian's own 1D density along
                    // the ray via core.h's random_normal(), not clamped to
                    // any fixed sigma range. See the sibling *_center.h /
