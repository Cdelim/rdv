...
FORWARD {
    ...
    // 1. SET UP AN ARRAY TO COLLECT HITS
    int hit_indices[64];
    float hit_depths[64];
    int hit_count = 0;
    ...
    while(rayQueryProceedEXT(rq)) {
        if (rayQueryGetIntersectionTypeEXT(rq, false) == gl_RayQueryCandidateIntersectionAABBEXT) {
            ...
            // Calculate where the center peak of the Gaussian is along the ray
            ...
            if (A > 1e-6) {
                float t_peak = -B / A; // The exact depth of the Gaussian center
                
                // If it is in front of the camera, save it!
                if (t_peak > 0.0 && hit_count < 64) {
    ...
    // A simple bubble sort for the shader array
    ...
    // --- PHASE 3: ANALYTICAL ALPHA BLENDING ---
    vec3 final_color = vec3(0.0);
    float T = 1.0; // Transmittance starts at 1.0 (fully clear)

    for (int h = 0; h < hit_count; ++h) {
        if (T < 0.01) break; // Early exit if ray is blocked!

        int i = hit_indices[h];
        
        // Recalculate A, B, C for this specific hit 
        vec3 d = x - positions.data[i]; 
        int cov_idx = i * 6;
        float M00 = inv_covs.data[cov_idx + 0];
        // ... (Load M01 through M22) ...
        float A = M00*w.x*w.x + M11*w.y*w.y + M22*w.z*w.z + 2.0 * (M01*w.x*w.y + M02*w.x*w.z + M12*w.y*w.z);
        ...
        if (power > -15.0 && power <= 0.0) {
            // THE ANALYTICAL INTEGRAL (replaces the Monte Carlo dice roll)
            float peak_density = exp(power);
            float integral = peak_density * sqrt(6.28318 / A); 
            
            float target_alpha = min(opacities.data[i], 0.999);
            float exact_tau = target_alpha * integral;
            float alpha = 1.0 - exp(-exact_tau);

            // Calculate Spherical Harmonics color...
            vec3 gaussian_color = colors.data[i] * sh_coefs[0]; 
            // (Add your f_rest loop here)
