FORWARD {
    ...
    while (rayQueryProceedEXT(rq)) {
        if (rayQueryGetIntersectionTypeEXT(rq, false) ==
            ...
            if (A > 1e-6) {
                float t_peak = -B / A;             // exact ray param of peak
                if (t_peak > 0.0) {
                    if (hit_count < MAX_HITS) {
                        ...
                        // keep the closest MAX_HITS: replace furthest
                        int   max_idx = 0;
    ...
    // ---- PHASE 2: SORT FRONT-TO-BACK (insertion sort) ----
    ...
    // ---- PHASE 3: ANALYTICAL FRONT-TO-BACK COMPOSITING (VPRF) ----
    vec3  final_color = vec3(0.0);
    float T = 1.0;

    for (int h = 0; h < hit_count; ++h) {
        if (T < 0.001) break;                      // opaque: stop
        ...
        if (A > 1e-6) {
            ...
            if (power > -15.0 && power <= 0.0) {
                ...
                float exact_tau = peak_tau * exp(power);   // paper: sqrt(2pi/A) cancels
                float alpha     = 1.0 - exp(-exact_tau);

                // skip negligible contributions (matches 3DGS 1/255 cutoff,
                // sharpens result and speeds up the loop)
    ...
    // Residual transmittance falls to background.
    // If the image looks uniformly dark, test with a background:
    // final_color += T * vec3(1.0);   // white bg test
