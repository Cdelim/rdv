...
class GS3D(_core.Map):
    ...
    def __init__(self, positions: _core.TensorLike | _core.deferred, colors:_core.TensorLike | _core.deferred, radius: float = 0.1, transform: _core.TensorLike | _core.deferred = None, input_dim=None, output_dim=None, input_requires_grad=False, bw_uses_output=False):
        ...
        if output_dim is None:
            output_dim = colors.shape[-1]  # distance + color
        assert output_dim == colors.shape[-1], f"GS3D output_dim must match the number of color channels, got output_dim={output_dim} but colors has shape {colors.shape}"
        assert colors.shape[-1] == 3, "TODO: to be changed"
    ...
    def build_geometry_ads(self, vk_ads_info=None, just_update=False, reuse=True, **deferred) -> dict:
        ...
        if not reuse or vk_ads_info is None:
            ...
            assert positions.shape[0] == vk_ads_info['num_primitives'], "Number of primitives in positions does not match number of primitives in vk_ads_info"
            aabb_buffer = vk_ads_info['aabb_buffer']
        # write bmin, bmax (using for example only a small box.
        aabb_buffer.load(_torch.cat([positions - self.radius, positions + self.radius], dim=-1))
        if not reuse or vk_ads_info is None:
            # create a geometry for the ADS build
